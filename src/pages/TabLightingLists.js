		...
		return rowData.map((item) => {
		  ...
		  if(item.category!==null){
		   cat = item.category.name
			...
			return(
			  <TouchableOpacity key={item.id} onPress={ () => {this.props.navigation.navigate('Details', {id: item.id})}}>
				<View style={[style.boxBorder, {marginBottom: 10}]}>
					<Text style={[style.h2,{marginBottom: 5, color:"#0B108C"}]}>{item.name}</Text>
					<Text style={[style.h3,{marginBottom: 5, position: "absolute", right: 0, marginTop: 37, marginRight: 12}]}>{item.category}</Text>
					<Text style={[style.h3,{marginBottom: 5}]}>Alamat : {item.address}</Text>
				</View>
			  </TouchableOpacity>
			);
		});
    ...
    render() {
        return (
            <View style={style.container}>
                ...
                <Header headerName="LIGHTING LISTS"></Header>
				<ScrollView>
